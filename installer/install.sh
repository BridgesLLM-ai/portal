...
build_portal() {
  ...
  if [[ -d "${RELEASE_FALLBACK_DIR}" ]] && [[ -f "${RELEASE_FALLBACK_DIR}/backend/package.json" ]]; then
    ...
    spin "Downloading portal release" "mkdir -p /tmp/blp-download && curl -fsSL '${RELEASE_URL}' -o /tmp/blp-download/portal.tar.gz && tar -xzf /tmp/blp-download/portal.tar.gz -C /tmp/blp-download && rsync -a --exclude='/projects' --exclude='/assets' --exclude='/.data' /tmp/blp-download/portal/ '${PORTAL_DIR}/' && rm -rf /tmp/blp-download"
  else
...
do_update() {
  ...
  if curl -fsSL --head "${RELEASE_URL}" &>/dev/null 2>&1; then
    spin "Downloading update" "mkdir -p /tmp/blp-download && curl -fsSL '${RELEASE_URL}' -o /tmp/blp-download/portal.tar.gz && tar -xzf /tmp/blp-download/portal.tar.gz -C /tmp/blp-download && rsync -a --delete --exclude='/projects' --exclude='/assets' --exclude='/upload-temp' --exclude='/.data' --exclude='.env.production' --exclude='.env' /tmp/blp-download/portal/ '${PORTAL_DIR}/' && rm -rf /tmp/blp-download"
    update_applied=true
