...
# Pinned versions
readonly PIN_OPENCLAW_VERSION="2026.7.1"
...
# silently downgrade a working HTTPS site to an HTTP-only config. Runs before
# env generation too, so .env.production DOMAIN matches the served vhost.
...
write_caddy_config() {
  ...
  if [[ -n "$DOMAIN" ]]; then
    # No splash/marketing handler here: that block referenced a docroot that
    # only exists on the project's own web host, and made every customer
    # install serve 404 on / for logged-out visitors. The portal serves its
    # own landing page.
...
build_portal() {
  ...
  if [[ -d "${RELEASE_FALLBACK_DIR}" ]] && [[ -f "${RELEASE_FALLBACK_DIR}/backend/package.json" ]] \
    ...
    spin "Copying portal files from local source" "rsync -a --delete --exclude='node_modules' --exclude='.git' --exclude='.env' --exclude='.env.production' --exclude='/projects' --exclude='/assets' --exclude='/upload-temp' --exclude='/.data' --exclude='/backend/.data' '${RELEASE_FALLBACK_DIR}/' '${PORTAL_DIR}/'"
  ...
  [[ -n "${OPENCLAW_TOKEN}" ]] || OPENCLAW_TOKEN="$(rand_hex 24)"
  # Mint a setup token only for genuinely fresh installs. A reinstall over an
  # existing account store (preserved env/DB) must not resurrect the setup
  # wizard: a fresh token there replaces the login page with the
  # password-reset flow for every logged-out visitor.
  if [[ -z "${SETUP_TOKEN}" && -z "${existing_database_url}" ]]; then
  ...
  # Write .env.production. Recover the served domain first: reinstalls reach
  # this point with an empty $DOMAIN while Caddy still serves the real vhost,
  # and DOMAIN/CORS/PORTAL_URL below must match it.
  recover_domain_from_caddyfile
  local cors_origin
...
NODE_ENV=production
PORT=4001
# Loopback bind: external access is served by Caddy over HTTPS. Widen only if
# you know you need the portal reachable without the reverse proxy.
HOST=127.0.0.1
...
do_update() {
  ...
  # /.data only matches the portal-root data dir; backend/.data holds persisted
  # runtime-turn events (chat history overlays, stream recovery). The release
  # tarball ships neither, so without its own exclude, --delete wipes all
  # persisted chat runtime history on every update.
  spin "Syncing updated portal files" "rsync -a --delete --exclude='node_modules' --exclude='.git' --exclude='.env' --exclude='.env.production' --exclude='/projects' --exclude='/assets' --exclude='/upload-temp' --exclude='/.data' --exclude='/backend/.data' '${staged_update_dir}/' '${PORTAL_DIR}/'"

  # Restore config
  cp "${backup_dir}/.env.production" "${PORTAL_DIR}/backend/" 2>/dev/null || true
  cp "${backup_dir}/.env" "${PORTAL_DIR}/frontend/" 2>/dev/null || true

  # Hardening repairs (idempotent):
  # - Older installs never set HOST, so the backend bound 0.0.0.0 and relied
  #   entirely on the host firewall. External access belongs to Caddy/HTTPS.
  # - backend/.data (persisted chat runtime events) predates its 700/600 modes.
  if [[ -f "${PORTAL_DIR}/backend/.env.production" ]] && ! grep -q '^HOST=' "${PORTAL_DIR}/backend/.env.production"; then
    printf '\n# Loopback bind (installer hardening): external access is served by Caddy.\nHOST=127.0.0.1\n' >> "${PORTAL_DIR}/backend/.env.production"
    info "Portal now binds to 127.0.0.1 (external access stays on Caddy/HTTPS)"
  fi
  if [[ -d "${PORTAL_DIR}/backend/.data" ]]; then
    find "${PORTAL_DIR}/backend/.data" -type d -exec chmod 700 {} + 2>/dev/null || true
    find "${PORTAL_DIR}/backend/.data" -type f -exec chmod 600 {} + 2>/dev/null || true
  fi
