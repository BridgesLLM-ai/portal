  ...
  },
  {
    ...
    checks: [
      ...
      },
      {
        ...
        label: 'Shared Chrome desktop entry',
        type: 'path',
        required: false,
        path: '/home/bridgesrd/Desktop/Shared Chrome.desktop',
        remediation: 'Re-run remote desktop setup to create the Shared Chrome desktop shortcut for the remote desktop user.',
      },
      {
  ...
  const matrix: FeatureReadinessDef[] = FEATURE_READINESS_MATRIX.map((feature) => {
    if (feature.id === 'remoteDesktop') {
      ...
      dynamicChecks.unshift({
        ...
        required: true,
        url,
        remediation: 'Set remoteDesktop.url in Settings → System so Desktop can connect (default: /novnc/vnc_portal.html?reconnect=1&resize=smart&path=novnc/websockify).',
      });
