...
import { gatewayRpcCall, patchSessionModel, getSessionInfo, isGatewayTransportError } from '../utils/openclawGatewayRpc';
import {
...
async function listOpenClawAgentsForSelector(): Promise<any[]> {
  const rpcResult = await gatewayRpcCall('agents.list', {});
      ...
      try {
        ...
        if (role === 'assistant') {
          if (Array.isArray(content)) {
            ...
            let displayText = '';
            if (hasToolCalls) {
              // Only keep text that appears AFTER the last tool call.
              // Text before tool calls is intermediate narration/thinking.
              const lastToolIdx = allBlocks.reduce((acc, b, i) => b.type === 'tool' ? i : acc, -1);
              const afterToolTexts = allBlocks
                .slice(lastToolIdx + 1)
                .filter(b => b.type === 'text')
                .map(b => b.text!)
                .join('\n');
              displayText = afterToolTexts;
            } else {
              displayText = allBlocks
                .filter(b => b.type === 'text')
                .map(b => b.text!)
                .join('\n');
            }
            const text = extractSanitizedText(displayText);
            return {
              id: entry.id,
              role: 'assistant',
              content: text,
...
router.post('/exec-approval/resolve', authenticateToken, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  ...
  if (!decision || !['approve', 'deny', 'always'].includes(decision)) { res.status(400).json({ error: 'Invalid decision' }); return; }
  try {
    if (isPersistentWsConnected()) {
...
async function handleWsExecApproval(ws: WebSocket, msg: any, user?: JwtPayload) {
  ...
  if (!approvalId || !['approve', 'deny', 'always'].includes(decision)) {
