...
const ACTIONS: Record<string, MaintenanceAction & { command: string; title: string }> = {
  ...
  'update-compatible-ai-tools': {
    ...
    description: 'Updates OpenClaw, Codex, Claude Code, ClawHub, Ollama, and supported native chat harnesses to the versions qualified for this Portal release. Also updates Agent Zero when configured.',
    risk: 'scheduled',
    downtimeExpected: true,
    requiresOwner: true,
    changesSystem: true,
    ...
    requiresMaintenanceWindow: true,
    automationLevel: 'guarded',
    impact: 'Pauses the OpenClaw bundle during its coordinated update, then updates the other harnesses separately. Pause chat activity for this maintenance window. Provider logins are preserved, not created.',
    recovery: 'OpenClaw bundle failure restores its previous coordinated bundle. Later harness failures stop maintenance but keep earlier verified updates; retry after resolving the reported tool error. Agent Zero retains its managed data and rollback snapshot.',
