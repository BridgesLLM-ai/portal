...
export default function DesktopPage() {
  ...
  const connectAudio = useCallback(() => {
    ...
    ws.onclose = (ev) => {
      ...
      if (audioEnabledRef.current) {
        const delay = ev.code === 1000 ? 0 : 3000;
        if (ev.code !== 1000) {
  ...
  const toggleFullscreen = () => {
    ...
    setTimeout(() => postViewportToIframe('fullscreen-toggle'), 500);
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`flex flex-col ${fullscreen ? 'fixed inset-0 z-50 bg-[#0A0E27]' : 'h-full'}`}
    >
      <div className="flex items-center justify-between px-2 sm:px-4 py-2 sm:py-3 border-b border-white/5 bg-[#0D1130]/80 backdrop-blur-xl flex-shrink-0 gap-3">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-[#0A0E27] z-10">
                <div className="text-center space-y-5 max-w-lg px-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <button onClick={runAutoSetup} disabled={setupRunning} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-5 py-3 text-sm font-medium text-white transition hover:bg-emerald-600 disabled:opacity-60 disabled:cursor-not-allowed">
                      {setupRunning ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Setting up...</> : <><Play size={16} /> Set Up Remote Desktop</>}
                    </button>
                    <button onClick={() => navigate('/settings')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-900 px-5 py-3 text-sm font-medium text-slate-300 transition hover:bg-slate-800 hover:text-white">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-[#0A0E27] z-10">
                <div className="text-center space-y-4 max-w-md px-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    ...
                    <button onClick={runAutoSetup} disabled={setupRunning} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 text-sm font-medium hover:bg-amber-500/20 transition-colors disabled:opacity-60">
