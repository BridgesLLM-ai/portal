...
function ShellTabSession({ tabId, isActive, onConnectionChange, onRunningChange, onDanger, onShowAssistant, acActiveRef, acSelectedIndexRef, setAcSuggestions, setAcSelectedIndex, setAcVisible, setAcInput }: {
  ...
  useEffect(() => {
    ...
    const socket = io(`${wsUrl}/terminal`, { transports: ['polling', 'websocket'], reconnection: false, withCredentials: true });
    ...
    socket.on('disconnect', (reason) => {
      ...
      captureError(new Error(`Terminal socket disconnected: ${reason}`), 'system', 'terminal websocket disconnect');
    ...
    socket.on('connect_error', (err: any) => {
      ...
      if (reconnectAttempt > 0 && reconnectAttempt < 10) {
...
export default function TerminalPage() {
  ...
  const [tabs, setTabs] = useState<TabDescriptor[]>([{ id: `tab-${Date.now()}`, label: 'bash', type: 'shell' }]);
