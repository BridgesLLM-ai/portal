      ...
      try {
        ...
        if (!cancelled && data.providers) {
          ...
          if (saved && usableProviders.some((p: ProviderInfo) => p.name === saved)) {
            onChange({ provider: saved, agentId: saved === 'OPENCLAW' ? (savedAgent || 'main') : undefined });
            ...
            if (!value || !usableProviders.some((p: ProviderInfo) => p.name === value)) {
              onChange({ provider: fallback, agentId: fallback === 'OPENCLAW' ? (savedAgent || 'main') : undefined });
            }
          }
        }
      } catch {
