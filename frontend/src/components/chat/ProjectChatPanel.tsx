...
export default function ProjectChatPanel({ projectName, onClose }: ProjectChatPanelProps) {
  ...
  const handleWsEvent = useCallback((data: any) => {
    ...
    switch (data.type) {
      ...
      case 'tool_end': {
        lastSegmentStartRef.current = assembledRef.current.length;
        setStatusText(null);
      ...
      case 'text': {
        const safeChunk = typeof data.content === 'string'
          ...
          : '';
        assembledRef.current = mergeAssistantStream(assembledRef.current, safeChunk, { replace: data.replace === true });
        const st = assembledRef.current.substring(lastSegmentStartRef.current);
        setStatusText(null);
      ...
      case 'stream_resume': {
        ...
        if (typeof data.content === 'string') {
          ...
          assembledRef.current = mergeAssistantStream(assembledRef.current, safeChunk, { replace: true });
          const st = assembledRef.current.substring(lastSegmentStartRef.current);
          const cid = streamingAssistantIdRef.current;
          setMessages(prev => prev.map(m => m.id === cid ? { ...m, content: st } : m));
        }
  ...
  }, [resetWatchdog, clearWatchdog, appendThinkingChunk]);
