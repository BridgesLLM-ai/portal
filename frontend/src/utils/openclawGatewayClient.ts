...
export class OpenClawGatewayClient {
  ...
  private scheduleReconnect(): void {
    ...
    console.log(`[OpenClawGatewayClient] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempt})`);
  ...
  async loadHistory(sessionKey: string, limit = 100): Promise<GatewayHistoryResponse> {
    const result = await this.request<{ messages: GatewayChatMessage[]; thinkingLevel?: string }>('session.history', {
      sessionKey,
      limit,
    });
  ...
  async subscribeSession(sessionKey: string): Promise<void> {
    await this.request('session.subscribe', { sessionKey });
  }
}
